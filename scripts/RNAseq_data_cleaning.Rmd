...
counts=read.delim('countMatrix.txt',sep=' ')
...
join GE pcs w/ metadata (see https://github.com/laylabrassington/rnaseq_genotyping)
```{r}
...
```{r}
meta$age_scale<-as.numeric(scale(meta$age))
meta$urb_scale<-as.numeric(scale(meta$urb_score))

v2=v[,meta$sample_name]
identical(meta$sample_name,colnames(v2))

design = model.matrix(~as.factor(meta$batch) + meta$Uniquely.mapped.reads + meta$monocytes_perc_combine + meta$lympho_perc_combine)
fit <-lmFit(v2,design)
fit <- eBayes(fit)
